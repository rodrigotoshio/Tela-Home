...
export const Iphone = () => {
  ...
  return (
    <div className="w-full max-w-[390px] min-h-screen bg-white mx-auto">
      ...
      <section className="relative w-full bg-[#2E96F1] text-white pb-20 pt-0">
        ...
        <div className="absolute inset-0 overflow-hidden">
          ...
          <div className="absolute top-48 left-16 w-1.5 h-1.5 bg-white/20 rounded-full"></div>
          <div className="absolute top-60 right-8 w-2 h-2 bg-white/25 rounded-full"></div>
          <div className="absolute bottom-32 left-12 w-1 h-1 bg-white/35 rounded-full"></div>
          <div className="absolute bottom-40 right-16 w-1.5 h-1.5 bg-white/30 rounded-full"></div>
        </div>

        {/* Ilustração central */}
        <div className="mt-[69px] px-[31px]">
          <div className="relative w-[359px] h-[223px] mx-auto">
            {/* FOLHA - Papel principal */}
            <div 
              className="absolute bg-white rounded-[7.41566px] w-[157.07px] h-[193.36px]"
              style={{
                left: '61px',
                top: '30px',
                transform: 'rotate(1.26deg)',
                boxShadow: '25.1551px 50.3102px 6.28877px rgba(0, 0, 0, 0.06)'
              }}
            >
              {/* ===== ELEMENTOS DENTRO DA FOLHA (posições ajustadas com base na imagem) ===== */}

              {/* PRIMEIRO ERRO (X e linha) */}
              {/* X Vermelho 1 */}
              <div 
                className="absolute w-[21.74px] h-[21.74px] bg-[#F74354] rounded-full flex items-center justify-center text-white font-bold text-sm"
                style={{
                  left: '20px', // Ajustado
                  top: '20px', // Ajustado
                  transform: 'rotate(0deg)' // Rotação para o X
                }}
              >
                ✗
              </div>
              {/* Linha Ondulada Vermelha 1 */}
              <div 
                className="absolute w-[42.46px] h-[2px] bg-[#F74354]/50 rounded" 
                style={{
                  left: '48px', // Ajustado
                  top: '28px', // Ajustado
                  transform: 'rotate(10deg)' // Rotação da linha
                }}
              ></div>

              {/* SEGUNDO ERRO (X e linha) */}
              {/* X Vermelho 2 */}
              <div 
                className="absolute w-[21.74px] h-[21.74px] bg-[#F74354] rounded-full flex items-center justify-center text-white font-bold text-sm"
                style={{
              ...
              {/* V Verde */}
              <div 
                className="absolute w-[30.08px] h-[30.08px] bg-[#B0EF8F] rounded-full flex items-center justify-center text-white font-bold text-lg"
                style={{
                  left: '15px', // Ajustado
                  top: '110px', // Ajustado
                  transform: 'rotate(0deg)' // Rotação para o V
                }}
              >
                ✓
              </div>
              {/* Linha Ondulada Verde */}
              <div 
                className="absolute w-[42.46px] h-[2px] bg-[#B0EF8F]/50 rounded" 
                style={{
                  left: '50px', // Ajustado
                  top: '125px', // Ajustado
                  transform: 'rotate(10deg)' // Rotação da linha
